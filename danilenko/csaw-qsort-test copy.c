...
void FillSaw(double *A, int N, double Min, double Max, int k)
{
...
typedef void (*FillFunctionType)(double *, int, double, double, int);
typedef void (*SortFunctionType)(double *, int);

#define NMax 5000000
#define Trials 1
#define FillFunctionsCount 1
#define SortFunctionsCount 1
int main()
{
    int t0, t1, time = 0;
  
    FillFunctionType FillFunctions[FillFunctionsCount] = {FillSaw};
    char FillFunctionsNames[FillFunctionsCount][10] = {"Saw\n"};
    SortFunctionType SortFunctions[SortFunctionsCount] = {TestQSort};
    
    double *A = malloc(sizeof(double) * NMax);
