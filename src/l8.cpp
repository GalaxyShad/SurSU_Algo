...
char EdgeToChar(int edge) {
    return 65 + edge;
}

void PrintPath(Matrix& m) 
{
    int n = m.Size();
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            ...
            printf("%c (%lf)-> %c; ", EdgeToChar(j), m(i, j), EdgeToChar(i));
        }
    }
...
void SolveTask(WGraph& graph, double pathLength)
{

    for (int i = 1; i < graph.Size(); i++) {
        Graph shortPathGraph = graph.ShortestPath(0, i);
        double len = GetLength(shortPathGraph);
        
        if (len <= pathLength) {
            printf( "A -> %c; %lf\n", EdgeToChar(i), len);
            PrintPath(shortPathGraph);
            printf("\n");
        }
